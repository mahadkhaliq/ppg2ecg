...
\begin{table}[H]
  ...
  \begin{tabular}{p{3.6cm} p{5.2cm} p{5.2cm}}
    ...
    Used for            & PPG$\to$ECG reconstruction       & AAMI 5-class beat classifier \\
    \bottomrule
  \end{tabular}
\end{table}
...
The trained BiLSTM is deployed in an interactive application (Figure~\ref{fig:ui}) demonstrating end-to-end use: PPG upload, sliding-window reconstruction, real-time R-peak detection, and AAMI five-class beat classification using a ResNet1D trained separately on MIT-BIH (99.4\% validation accuracy).

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{UI_ss.png}
  \caption{Interactive application interface (hospital-monitor style). The vitals bar reports instantaneous heart rate, rhythm label with confidence, RMSSD heart-rate variability, and beat count. The main panel shows a 60\,fps canvas-rendered ECG sweep with colour-coded beat markers (green: N, orange: S, red: V, purple: F, grey: Q). Beat classifier output is displayed below the trace; the screenshot captures a tachycardia segment (HR\,=\,126\,bpm) with all 126 beats classified as Normal.}
  \label{fig:ui}
\end{figure}
...
\textit{IEEE Eng. Med. Biol. Mag.}, vol.~20, no.~3, pp.~45--50, May/June 2001.
